...
-- changeset mkp:add-records-31032015-01
...
-- changeset mkp:add-records-31032015-02
...
MERGE INTO companies c
USING ( SELECT 1 FROM dual ) d
ON ( c.name = 'Airbus Group NV non-registered shares' )
WHEN NOT MATCHED THEN
INSERT (id,
name
)
VALUES (companies_seq.nextval,
'Airbus Group NV non-registered shares'
)
;

MERGE INTO assets a
USING ( SELECT 1 FROM dual ) d
ON ( a.asset_name = 'Airbus Group NV non-registered shares' )
WHEN NOT MATCHED THEN
INSERT (id,
asset_name,
company_id
...
VALUES (assets_seq.nextval,
'Airbus Group NV non-registered shares',
(SELECT id FROM companies WHERE name='Airbus Group NV non-registered shares')
)
;
...
VALUES (funds_portfolio_seq.nextval,
(SELECT id FROM funds WHERE name='fundamentalinvest'),
(SELECT id FROM assets WHERE asset_name='Airbus Group NV non-registered shares'),
281967,
4339253,
to_date(31032015, 'ddmmyyyy')
)
...
MERGE INTO companies c
USING ( SELECT 1 FROM dual ) d
ON ( c.name = 'California Resources Corp' )
WHEN NOT MATCHED THEN
INSERT (id,
name
)
...
MERGE INTO assets a
USING ( SELECT 1 FROM dual ) d
ON ( a.asset_name = 'California Resources Corp' )
WHEN NOT MATCHED THEN
INSERT (id,
asset_name,
company_id
...
VALUES (assets_seq.nextval,
'California Resources Corp',
(SELECT id FROM companies WHERE name='California Resources Corp')
)
;
...
VALUES (funds_portfolio_seq.nextval,
(SELECT id FROM funds WHERE name='fundamentalinvest'),
(SELECT id FROM assets WHERE asset_name='California Resources Corp'),
6088,
800000,
to_date(31032015, 'ddmmyyyy')
)
